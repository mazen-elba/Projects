...
"""
Renders tracks (tracking_msg/TrackedPersons) and odometry to an SVG file, (C)2014 Timm Linder
Parameters:
...
from tracking_msgs.msg import TrackedPersons, TrackedPerson, DetectedPersons, DetectedPerson
...
class TrackReceiver(object):
    def __init__(self):
        ...
        rospy.loginfo("Will start processing tracks at time index %f and end at %f" % (
            self.startLimit, self.endLimit))
        if self.autoQuit:
            rospy.loginfo(
                "Will end processing if no data is received for more than 10 seconds after last data.")

        rospy.loginfo(
            "Listening for tracks... press CTRL+C to stop and generate SVG output.")
    ...
    def newDetectionsReceived(self, detectedPersons):
        ...
        with self.mutex:
            ...
            for detectedPerson in detectedPersons.detections:
                ...
                try:
                    pos = self.transformIntoTargetFrame(
                        detectedPerson.pose, detectedPersons.header).pose.position
                except tf.Exception:
                    self.numDetectionTfErrors += 1
                    if self.numDetectionTfErrors < 2:
                        rospy.logwarn(
                            "TF exception encountered while transforming detection position into target frame!")
                    return
                ...
                if self.positionPassesSanityCheck(pos):
                    ...
                    rospy.logwarn(
                        "Ignoring DetectedPerson because its position does not pass heuristic sanity check:\n" + str(detectedPerson))
    ...
    def newTracksReceived(self, trackedPersons):
        ...
        with self.mutex:
            ...
            for trackedPerson in trackedPersons.tracks:
                ...
                try:
                    pos = self.transformIntoTargetFrame(
                        trackedPerson.pose, trackedPersons.header).pose.position
                except tf.Exception:
                    self.numTrackTfErrors += 1
                    if self.numTrackTfErrors < 2:
                        rospy.logwarn(
                            "TF exception encountered while transforming track position into target frame!")
                    return
                ...
                if self.positionPassesSanityCheck(pos):
                    ...
                    rospy.logwarn(
                        "Ignoring TrackedPerson because its position does not pass heuristic sanity check:\n" + str(trackedPerson))

            database.trackStamps.append(secs)

    def odometryReceived(self, odom):
        try:
            posInTargetFrame = self.transformIntoTargetFrame(
                odom.pose, odom.header).pose.position
            ...
            if self.numOdometryTfErrors < 2:
                rospy.logwarn(
                    "TF exception encountered while transforming odometry into target frame!")
            pass
    ...
    def spin(self):
        while not (quitRequested or rospy.is_shutdown() or rospy.get_time() > self.endLimit):
            ...
            if self.autoQuit and self.firstTracksReceived and not self.lastDataAt is None and time.time() - self.lastDataAt > 10.0:
                rospy.loginfo(
                    "No data received for more than 10 seconds, quitting.")
                break
        self.stop()
...
class TrackRenderer(object):
    ...
    def getTrackColor(self, track_id, trackCycle=0):
        ...
        if self.colormap == 'srl' or self.colormap == 'srlalternative':
            ...
            palette = [0xaf1f90, 0x000846, 0x00468a, 0x00953d, 0xb2c908, 0xfcd22a,
                       0xffa800, 0xff4500, 0xe0000b, 0xb22222]  # rainbow color map
    ...
    def animateElement(self, element, currentCycle, cycleStamps, showHistory=True):
        ...
        if currentCycle == 0:
            ...
            keyTimes += str((cycleStamps[currentCycle -
                                         1] - firstStamp) / duration)

        keyTimes += ";" + \
            str((cycleStamps[currentCycle] - firstStamp) / duration)

        if not showHistory:
            if currentCycle < len(cycleStamps) - 1:
                keyTimes += ";" + \
                    str((cycleStamps[currentCycle + 1] -
                         firstStamp) / duration) + ";1"
        ...
        animation = svg.animate(attributeName='opacity', values=opacityValues, keyTimes=keyTimes, calcMode='discrete', dur="%fs" % (
            duration / self.animationSpeed), repeatCount='indefinite', fill='freeze')
        element.add(animation)

    def renderGrid(self, left, top, width, height):
        pattern = svg.defs.add(svg.pattern(
            id="gridPattern", size=(1, 1), patternUnits="userSpaceOnUse"))
        pattern.add(svg.path(d="M 1 0 L 0 0 0 1", fill="none",
                             stroke="#CCC", stroke_width=0.01))
        content.add(svg.rect(id='background', insert=(
            left, top), size=(width, height), fill="white"))
        content.add(svg.rect(id='grid', insert=(left, top),
                             size=(width, height), fill="url(#gridPattern)"))

    def renderOdometry(self):
        if len(database.odom) > 0:
            content.add(svg.polyline(database.odom, id='odom', stroke='#999',
                                     stroke_width=0.02, stroke_linecap='round', fill='none'))
        ...
        if self.velocitiesEnabled or self.animationEnabled:
            ...
            for step in range(lookback, len(database.odom), lookback):
                ...
                deltatime = database.odomStamps[step] - \
                    database.odomStamps[step-lookback]
                vel = ((pos[0] - previousPos[0]) / deltatime,
                       (pos[1] - previousPos[1]) / deltatime)
                ...
                arrow = svg.polyline(arrowCoords, id='arrow', stroke_width=0.015,
                                     stroke_opacity=0.35, stroke='#999', fill='none')
    ...
    def renderVelocities(self):
        ...
        for track_id, track in database.tracks.iteritems():
            trackVelocityGroup = velocityGroup.add(
                svg.g(id='velocities%d' % track_id))
            ...
            for step in range(0, len(track.positions)):
                ...
                arrow = svg.polyline(arrowCoords, id='arrow', stroke_width=0.015,
                                     stroke_opacity=0.35, stroke=self.getTrackColor(track_id), fill='none')
                arrow.translate(pos)
                arrow.rotate(theta)

                self.animateElement(
                    arrow, track.cycles[step], database.trackStamps)
    ...
    def renderTrack(self, track_id, track):
        ...
        if len(track.positions) > 0:
            trackGroup.add(svg.polyline(track.positions, stroke=self.getTrackColor(
                track_id), stroke_width=0.01, stroke_linecap='round', fill='none'))
        ...
        if self.style == 'points':
            for step in range(0, len(track.positions)):
                ...
                if not track.occluded[step] or not self.showOcclusions:
                    trackVisual = svg.circle(
                        track.positions[step], r=0.1, fill=self.getTrackColor(track_id, trackCycle))

                else:
                    trackVisual = svg.circle(track.positions[step], r=0.09, fill='none', stroke=self.getTrackColor(
                        track_id, trackCycle), stroke_width=0.01)

                self.animateElement(trackVisual, trackCycle,
                                    database.trackStamps, self.historyEnabled)
    ...
    def renderDetections(self):
        ...
        for detection in database.detections:
            size = 0.075
            rect = svg.rect(size=(size, size), fill='none',
                            stroke='#999', stroke_width=0.005)
            rect.translate(
                (detection.position[0] - 0.5*size, detection.position[1] - 0.5*size))
            rect.rotate(45)

            self.animateElement(rect, detection.cycle,
                                database.detectionStamps, self.historyEnabled)
    ...
    def renderTrackLabels(self):
        ...
        for track_id, track in database.tracks.iteritems():
            ...
            if len(track.positions) > 0:
                ...
                fontSize = 0.25
                labelGroup.add(svg.text(str(track_id), insert=pos, text_anchor="middle",
                                        fill=trackColor, stroke='white', stroke_width=0.02, font_size=fontSize))
                labelGroup.add(svg.text(str(track_id), insert=pos,
                                        text_anchor="middle", fill=trackColor, font_size=fontSize))
    ...
    def renderTimestamps(self):
        ...
        for track_id, track in database.tracks.iteritems():
            ...
            timestamp = "+%.1f" % (track.createdAt -
                                   database.firstDataReceivedAt)
            creationTimestampGroup = content.add(svg.g())
            creationTimestampGroup.add(svg.text(timestamp, insert=pos, text_anchor="middle",
                                                fill=trackColor, stroke='white', stroke_width=0.02, font_size=fontSize))
            creationTimestampGroup.add(svg.text(
                timestamp, insert=pos, text_anchor="middle", fill=trackColor, font_size=fontSize))
            ...
            pos = track.positions[-1]
            pos = (pos[0], pos[1] + 0.17)
            timestamp = "-%.1f" % (track.deletedAt -
                                   database.firstDataReceivedAt)
            deletionTimestampGroup = content.add(svg.g())
            deletionTimestampGroup.add(svg.text(timestamp, insert=pos, text_anchor="middle",
                                                fill=trackColor, stroke='white', stroke_width=0.02, font_size=fontSize))
            deletionTimestampGroup.add(svg.text(
                timestamp, insert=pos, text_anchor="middle", fill=trackColor, font_size=fontSize))
    ...
    def renderCycles(self):
        ...
        for track_id, track in database.tracks.iteritems():
            ...
            for i in xrange(0, len(track.positions), 2):
                ...
                cyclesGroup.add(svg.text(cycleText, insert=pos, text_anchor="middle",
                                         fill=trackColor, stroke='white', stroke_width=0.02, font_size=fontSize))
                cyclesGroup.add(svg.text(
                    cycleText, insert=pos, text_anchor="middle", fill=trackColor, font_size=fontSize))
    ...
    def generateSVG(self):
        ...
        global svg, content
        filename = '%s%s.svg' % (
            self.filenamePrefix, time.strftime("%Y-%m-%d %H:%M:%S"))
        svg = svgwrite.Drawing(filename, profile='full', size=('%f' % (
            150*width), '%f' % (150*height)), viewBox=('%f %f %f %f' % (left, top, width, height)))
        ...
        if not self.animationEnabled:
            ...
            htmlTemplateFilename = roslib.packages.get_pkg_dir(
                'srl_tracking_exporter') + "/scripts/svg_frame.html"

            with open(htmlTemplateFilename, "r") as htmlTemplate:
                htmlContent = htmlTemplate.read()

            svgXml = svg.tostring()
            htmlOutput = htmlContent.replace(
                "<!-- ${INSERT_SVG_HERE} -->", svgXml)
        ...
        try:
            ...
            rospy.loginfo(
                "Failed to notify job monitor (maybe not running?): %s" % e)
    ...
    while not quitRequested:
        try:
            ...
            if not messageShown:
                messageShown = True
                print(
                    "Waiting for ROS master... please make sure that roscore is running!")
            time.sleep(0.1)
    sys.exit(0)
