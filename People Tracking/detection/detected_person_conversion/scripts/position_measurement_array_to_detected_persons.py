...
"""
Converts a people_msgs/PositionMeasurementArray into tracking_msgs/DetectedPersons which can be processed using a tracker, or visualized using
tracking_rviz_plugin. This conversion is lossless except for the co-occurrence matrix and the object identifier string.
"""
import rospy
import re
...
def newMessageReceived(positionMeasurementArray):
    ...
    for positionMeasurement in positionMeasurementArray.people:
        ...
        for x in xrange(0, 3):
            for y in xrange(0, 3):
                detectedPerson.pose.covariance[y*6 +
...
rospy.loginfo("Re-publishing people_msgs/PositionMeasurementArray from %s as tracking_msgs/DetectedPersons at %s" %
