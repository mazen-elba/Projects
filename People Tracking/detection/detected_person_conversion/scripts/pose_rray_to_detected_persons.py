...
"""
Converts a geometry_msgs/PoseArray into tracking_msgs/DetectedPersons which can be processed using a tracker, or visualized using
tracking_rviz_plugin. This conversion is lossless.
"""
import rospy
from tracking_msgs.msg import DetectedPersons, DetectedPerson
...
def newMessageReceived(poseArray):
    ...
    for pose in poseArray.poses:
        ...
        for i in xrange(0, 6):
            detectedPerson.pose.covariance[i*6 +
                                           i] = posVariance if i < 3 else rotVariance
...
rospy.loginfo("Re-publishing geometry_msgs/PoseArray from %s as tracking_msgs/DetectedPersons at %s, with a positional / rotational variance of %.2f / %.2f and modality '%s', confidence %.2f, ID increment %d and offset %d"
