...
Example of usage:
trackAssociation = detected_person_association.TrackAssociation()
...
class TrackAssociation(object):
    ...
    def __init__(self, maxFusedDetectionsToRemember=1000, maxOriginalDetectionsToRemember=10000):
        ...
        self.fusedDetectionIdMemory = FusedDetectionIdMemory(
            maxOriginalDetectionsToRemember)
        ...
        with self.lock:
            ...
            if trackId is None:
                ...
                if not self.trackLookup:
                    rospy.logwarn(
                        "Lookup from detections to tracks is empty, make sure %s is being published!" % self.trackSubscriber.topicName)
                else:
                    ...
                    fusedDetectionId = self.fusedDetectionIdMemory.lookupFusedDetectionId(
                        detectionId)
...
class FusedDetectionIdMemory(object):
    ...
    The fused detection ID is the one referenced in tracking_msgs/TrackedPerson messages.
    """
    ...
    def lookupFusedDetectionId(self, originalDetectionId):
        with self.lock:
            if not self.fusedDetectionLookup and not self.noDataWarningShown:
                ...
                rospy.logwarn(
                    "Lookup from original detection IDs to fused detection IDs is empty, make sure %s is being published!" % self.fusedDetectionsTopic)
...
class TrackSynchronizer(message_filters.SimpleFilter):
    def __init__(self, inputFilter, queue_size):
        ...
        self.trackAssociation.trackSubscriber.addCallback(
            self.newTracksAvailable)
    ...
    def newMessage(self, msg):
        with self.lock:
            ...
            if numMessagesDeleted > 0:
                rospy.loginfo(
                    "TrackSynchronizer: %d buffered message(s) had to be deleted because no recent tracked person information was available!")
